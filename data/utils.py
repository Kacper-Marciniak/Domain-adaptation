...
def match_histogram(image, reference):
    assert len(image.shape) == len(reference.shape), 'Inputs must have same dim'
    if len(image.shape)==2: channel_axis = None
    else: channel_axis = 2
    return match_histograms(image, reference, channel_axis=channel_axis)
